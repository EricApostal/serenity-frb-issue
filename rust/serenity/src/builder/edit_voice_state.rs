...
pub struct EditVoiceState {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    request_to_speak_timestamp: Option<Box<Option<Timestamp>>>,
}
...
impl EditVoiceState {
    ...
    pub fn request_to_speak(mut self, request: bool) -> Self {
        self.request_to_speak_timestamp = Some(request.then(Timestamp::now).into());
        self
    }
    ...
    pub fn request_to_speak_timestamp(mut self, timestamp: impl Into<Timestamp>) -> Self {
        self.request_to_speak_timestamp = Some(Some(timestamp.into()).into());
        self
    }
}
